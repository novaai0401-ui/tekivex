...
The headless model is what lets the same engine power a stripped-down single-field form and a full annotation suite without compromise. It runs entirely client-side, so a custom viewer built this way inherits Pyntra's privacy and latency properties for free — see [the client-side approach](/use-cases/pyntra-client-side-pdf-editing), which includes the end-to-end fill-and-sign walkthrough, or browse the [use-cases hub](/use-cases) and the [Pyntra product page](/product/pyntra) for the full picture. Bring your own UI; let the engine handle the PDF.
